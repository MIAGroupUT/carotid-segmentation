...
from carotid.train.heatmap.utils import compute_centerline_df
    ...
    write_json(
        {
            ...
            "data": {"raw_dir": raw_dir, "centerline_dir": centerline_dir},
        },
        path.join(output_dir, "parameters.json"),
    )

    if centerline_tsv is not None:
        centerline_df = pd.read_csv(centerline_tsv, sep="\t")
    else:
        centerline_df = compute_centerline_df(
            raw_dir=raw_dir, centerline_dir=centerline_dir
        )

    # Generate splits
    participants = centerline_df.participant_id.unique()
