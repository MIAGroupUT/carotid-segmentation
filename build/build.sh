...
case $1 in
  'test')
    docker build -t carotidsegmentation-test -f build/debian.docker --build-arg MODELPATH="tests/models/" $SCRIPTPATH
    ;;
  'test-grand-challenge')
    ...
    ;;
  'grand-challenge')
    docker build -t carotidsegmentation-grandchallenge -f build/grand-challenge.docker --build-arg MODELPATH="models/" $SCRIPTPATH
    ;;
  *)
